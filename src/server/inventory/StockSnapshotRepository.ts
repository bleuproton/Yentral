...
export class StockSnapshotRepository {
  ...
  upsert(tenantId: string, warehouseId: string, variantId: string, onHand: number, reserved: number) {
  ...
  async upsertAndApplyDelta(
    ...
    return this.upsert(tenantId, warehouseId, variantId, onHand, reserved);
  }
}
