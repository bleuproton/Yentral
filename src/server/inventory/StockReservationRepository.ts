...
export class StockReservationRepository {
  ...
  updateStatus(tenantId: string, reservationId: string, status: ReservationStatus) {
    return this.db.stockReservation.updateMany({ where: { id: reservationId, tenantId }, data: { status } });
  }

  sumActiveQty(tenantId: string, warehouseId: string, variantId: string) {
    return this.db.stockReservation.aggregate({
